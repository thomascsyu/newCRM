...
import frappe
from crm.security.workspace import company_email
from crm.company_auth import configuration
...
class CRMInvitation(Document):
	...
	def before_insert(self):
		self.email = company_email(self.email, configuration()[0])
		frappe.only_for(["System Manager", "Sales Manager"])
		if self.role not in ("Sales User", "Sales Manager", "System Manager"):
			frappe.throw(_("Invalid CRM role"))
	...
	def invite_via_email(self):
		invite_link = configuration()[1] + "/company-login"
	...
	def accept(self):
		company_email(self.email, configuration()[0])
		if self.role != "Sales User":
			frappe.only_for("System Manager")
