...
from crm.api.calls import get_contact_by_phone_number
...
class CRMCallLog(Document):
	...
	def validate(self):
		if self.recording_url:
			file_name = frappe.db.get_value("File", {"file_url": self.recording_url,
				"is_private": 1, "attached_to_doctype": self.doctype, "attached_to_name": self.name})
			if not file_name or not self.recording_url.startswith("/private/files/"):
				frappe.throw(_("Recordings must be private files attached to this call log."))
