...
| Invalid or expired Google sign-in immediately after choosing an account | Deploy the signed-cookie binding fix following PR #16, then start a fresh attempt from `/company-login`. Anonymous sessions have `sid=Guest`; the callback must verify the signed OAuth cookie and compare its inner binding, not hash the whole envelope. The cookie signature is decoded by its fixed length, and its state expires server-side after 10 minutes. CRM Error Log entries titled `Google OAuth state` distinguish missing/invalid state from a browser binding mismatch without recording cookie contents. |
| Google sign-in could not be verified | Usually a bad or stale authorization code, or Google identity lookup failed after token exchange. Start again from `/company-login`. If it persists, confirm `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` have no trailing whitespace and match the Google Cloud OAuth client, and check CRM Error Log entries titled `Google OAuth`. |
| Use your company Google Workspace account | The Google account is not a managed `@gabriel.hk` Workspace user. Sign in with the company account, not a personal Gmail address. |
